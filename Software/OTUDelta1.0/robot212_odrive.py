...
Nm2A = 0.0000369#0604 #N*m/radian to A/count
...
CPR2RAD = (2*math.pi/65536)#400000)

odrvs = [None, None]
'''[[ODrive 0, ODrive 1]]'''
usb_serials = ['20583883304E', '205739824D4D']
axes = [None, None, None]
axis0 = None
axis1 = None
axis2 = None
...
def rad2Count(angle):
    try:
        return [ang/CPR2RAD for ang in angle] #100000-ang
    except TypeError:
        return angle/CPR2RAD #100000-ang
...
def count2Rad(count):
    try:
        return [(cnt)*CPR2RAD for cnt in count] #100000-cnt
    except TypeError:
        return (count)*CPR2RAD #100000-count
...
# def printFKPos():
#     ii = 0
#     axis0_encoder_pos = odrvs[0].axis0.encoder.pos_estimate
#     axis1_encoder_pos = odrvs[0].axis1.encoder.pos_estimate
#     axis2_encoder_pos = odrvs[1].axis0.encoder.pos_estimate
#     encoder_rad0 = CPR2RAD*axis0_encoder_pos
#     encoder_rad1 = CPR2RAD*axis1_encoder_pos
#     encoder_rad2 = CPR2RAD*axis2_encoder_pos
#     theta = [encoder_rad0, encoder_rad1, encoder_rad2]
#     print(theta)
#     # IKpos = deltaKin.FK(theta)
#     return  theta
    # print(IKpos)
...
def vel_test_all(amt = 30000, mytime = 2):
    ...
    #print_all()
############
def pos_set_point(posDesired = (10000, 10000, 10000)):
    for ii in range(0,3):
        axis = axes[ii]
        axis.controller.pos_setpoint = (rad2Count(-posDesired[ii]))
############
...
def set_gainsCounts(k_p, k_d, k_i, perm = True):
    for axis in axes:
        if(k_d != 0):
            axis.controller.config.pos_gain = k_p
            axis.controller.config.vel_gain = k_d
            axis.controller.config.vel_integrator_gain = k_i
    if(perm):
        odrvs[0].save_configuration()
        odrvs[1].save_configuration()
        time.sleep(2)
